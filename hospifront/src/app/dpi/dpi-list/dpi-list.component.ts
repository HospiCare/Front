...
export class DPIListComponent implements OnInit {
      ...
      next: (response) => {
        ...
        if (data && data.success) {
          ...
          this.dossiers = [{
            ...
            name: patient?.name ,
            email: patient?.email ,
            phone: data.phone ,  // Accès à dpi.phone
            creationDate: data.creationDate?.split(' ')[0] ,  // Accès à dpi.creationDate
            creationTime: data.creationDate?.split(' ')[1],
            date_naissance: patient?.date_naissance,
            mutuelle: data.mutuelle,
            personneContact: data.phone,
          }];
  ...
  navigateToPatientDetails(patient: Patient) {
    ...
    const patientDetails = {
      ...
      tel: patient.phone,
      mutuelle: patient.mutuelle, // You'll need to add this to your patient interface if available
      personneContact: patient.personneContact, // You'll need to add this to your patient interface if available
    };
